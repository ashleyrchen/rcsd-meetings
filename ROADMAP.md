...
- [ ] **"Dial the District" voice agent** — two phone numbers (English and Spanish) that parents can call to ask questions about the district and get spoken answers. Backed by district data (same JSON as the MCP server). Explore **Gemini 3.1 Flash Live** (https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-1-flash-live/) as the voice model -- native voice-to-voice avoids the latency and cost of a STT→LLM→TTS pipeline. Need to evaluate: does Flash Live support function/tool calling so it can query district data? Telephony integration (Twilio SIP trunk or similar) to connect phone calls to the model. Separate assistant configs for EN and ES.
